...
async def measure_average_delay(target_ip, target_port, num_samples=5):
    ...
    for _ in range(num_samples):
        ...
        try:
            ...
            delays.append(1000)  # Zaman aşımı durumunda yüksek gecikme eklenir
        finally:
            if 'writer' in locals():
                writer.close()
...
async def calculate_base_delays():
    base_delays[("A", "B")] = await measure_average_delay(NEIGHBORS["B"][0], NEIGHBORS["B"][1])
    base_delays[("A", "C")] = await measure_average_delay(NEIGHBORS["C"][0], NEIGHBORS["C"][1])
