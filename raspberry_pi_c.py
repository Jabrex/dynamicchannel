...
# Configuration
DEVICE_IP = "192.168.1.103"  # Current device's IP
DEVICE_PORT = 12345
NEIGHBORS = {
    "D": ("192.168.1.104", 12345),
    "E": ("192.168.1.105", 12345),  # Target device
}
...
epsilon = 0.1
Q = {
    "C": {"D": 0.0, "E": 0.0},  # Initial Q-values for state C
}
...
async def calculate_base_delays():
    ...
    for neighbor, (ip, port) in NEIGHBORS.items():
        try:
            base_delays[("C", neighbor)] = await measure_average_delay(ip, port)
        except Exception as e:
            base_delays[("C", neighbor)] = 1000  # High default delay
...
async def handle_connection(reader, writer):
    ...
    try:
        ...
        # En iyi rotayı bul ve paketi hedefe kadar yönlendir
        state = "C"
        while state != "E":  # "E" is the target device
        ...
        print("Packet has reached the target device (192.168.1.105).")
    except Exception as e:
